 ...
 createRental: (req, res) => {
    ...
    staffService.createRental(customer_id, inventory_id, staff_id, (err, rental) => {
      if (err) {
        ...
        return res.redirect(`/staff/rentalCreate?error=${encodeURIComponent('Kon verhuur niet aanmaken: ' + err.message)}`);
      }
      
      // Get customer and film details for success message
      res.redirect(`/staff/rentals?success=${encodeURIComponent('Verhuur succesvol aangemaakt!')}&type=rental_created`);
    });
  },
  ...
  createCustomer: (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hashedPassword) => {
      if (err) {
        return res.redirect(`/staff/customers?error=${encodeURIComponent('Kon wachtwoord niet hashen')}`);
      }
      ...
      staffService.createCustomer(
        ...
        (err, result) => {
          if (err) {
            return res.redirect(`/staff/customers?error=${encodeURIComponent(err.message)}`);
          }
          
          // Success redirect with popup
          const customerName = `${first_name} ${last_name}`;
          res.redirect(`/staff/customers?success=${encodeURIComponent(customerName)}&type=customer_created`);
        }
      );
    });
  },
  ...
  updateCustomer: (req, res) => {
    ...
    staffService.updateCustomer(customerId, customerData, (err, result) => {
      if (err) {
        ...
        return res.redirect(`/staff/customer/${customerId}/edit?error=${encodeURIComponent(err.message)}`);
      }
      
      // Success redirect with popup
      const customerName = `${first_name} ${last_name}`;
      res.redirect(`/staff/customers?success=${encodeURIComponent(customerName)}&type=customer_updated`);
    });
  },

  // Alle andere methodes blijven hetzelfde...
  dashboard: (req, res) => {
    const staffId = req.session.user.staff_id || 1;
  ...
  returnRental: (req, res) => {
    ...
    staffService.returnRental(rentalId, staff_id, (err, result) => {
      ...
      res.json({ success: true, message: 'Film succesvol geretourneerd' });
    });
  },
